...
ORDERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/ordererOrganizations/healthcare.com/orderers/orderer.healthcare.com/msp/tlscacerts/tlsca.healthcare.com-cert.pem
PEER0_HPROVIDER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/peerOrganizations/hprovider.healthcare.com/peers/peer0.hprovider.healthcare.com/tls/ca.crt
PEER0_RESEARCH_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/peerOrganizations/research.healthcare.com/peers/peer0.research.healthcare.com/tls/ca.crt
PEER0_ORG3_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/peerOrganizations/org3.healthcare.com/peers/peer0.org3.healthcare.com/tls/ca.crt
...
setOrdererGlobals() {
  ...
  CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/ordererOrganizations/healthcare.com/orderers/orderer.healthcare.com/msp/tlscacerts/tlsca.healthcare.com-cert.pem
...
setGlobals() {
  ...
  if [ $ORG -eq 1 ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/peerOrganizations/research.healthcare.com/users/Admin@research.healthcare.com/msp
    if [ $PEER -eq 0 ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/peerOrganizations/org3.healthcare.com/users/Admin@org3.healthcare.com/msp
    if [ $PEER -eq 0 ]; then
...
joinChannelWithRetry() {
  ...
  verifyResult $res "After $MAX_RETRY attempts, peer${PEER}.${org_name[$ORG-1]} has failed to join channel '$CHANNEL_NAME' "
}
...
installChaincode() {
  ...
  VERSION=${3:-1.0}
  set -x
  peer chaincode install -n dicom -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} >&log.txt
  res=$?
  set +x
  cat log.txt
...
chaincodeQuery() {
  ...
  setGlobals $PEER $ORG
  EXPECTED_RESULT=$3
  echo "===================== Querying on peer${PEER}.${org_name[$ORG-1]} on channel '$CHANNEL_NAME'... ===================== "
  local rc=1
  ...
  do
    sleep $DELAY
    echo "Attempting to Query peer${PEER}.${org_name[$ORG-1]} ...$(($(date +%s) - starttime)) secs"
    set -x
    peer chaincode query -C $CHANNEL_NAME -n dicom -c '{"Args":["query","a"]}' >&log.txt
    res=$?
    set +x
  ...
  if test $rc -eq 0; then
    echo "===================== Query successful on peer${PEER}.${org_name[$ORG-1]} on channel '$CHANNEL_NAME' ===================== "
  else
    echo "!!!!!!!!!!!!!!! Query result on peer${PEER}.${org_name[$ORG-1]} is INVALID !!!!!!!!!!!!!!!!"
    echo "================== ERROR !!! FAILED to execute End-2-End Scenario =================="
    echo
    exit 1
  fi
