...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"healthchannel\")"
  ...
  echo
  echo "	byfn.sh generate -c healthchannel"
  echo "	byfn.sh up -c healthchannel -s couchdb"
  echo "        byfn.sh up -c healthchannel -s couchdb -i 1.4.0"
  echo "	byfn.sh up -l node"
  echo "	byfn.sh down -c healthchannel"
  echo "        byfn.sh upgrade -c healthchannel"
  echo
...
function networkDown() {
  # stop org3 containers also in addition to hprovider and research, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove the docker-compose yaml file that was customized to the healthcare
...
function generateCerts() {
  ...
  echo
  echo "Generate CCP files for HProvider and Research"
  ./ccp-generate.sh
}
...
# and two Peer Orgs (``HProvider`` & ``Research``) each managing and maintaining two peer nodes.
# This file also specifies a consortium - ``HealthConsortium`` - consisting of our
# two Peer Orgs.  Pay specific attention to the "Profiles" section at the top of
# this file.  You will notice that we have two unique headers. One for the orderer genesis
# block - ``HealthOrgsOrdererGenesis`` - and one for our channel - ``HealthOrgsChannel``.
...
function generateChannelArtifacts() {
  ...
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
    configtxgen -profile HealthOrgsOrdererGenesis -channelID $SYS_CHANNEL -outputBlock ./channel-artifacts/genesis.block
  elif [ "$CONSENSUS_TYPE" == "kafka" ]; then
    configtxgen -profile HealthDevModeKafka -channelID $SYS_CHANNEL -outputBlock ./channel-artifacts/genesis.block
  elif [ "$CONSENSUS_TYPE" == "etcdraft" ]; then
    configtxgen -profile HealthMultiNodeEtcdRaft -channelID $SYS_CHANNEL -outputBlock ./channel-artifacts/genesis.block
  else
    set +x
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile HealthOrgsChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile HealthOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/HProviderMSPanchors.tx -channelID $CHANNEL_NAME -asOrg HProviderMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for HProviderMSP..."
    exit 1
  fi
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for ResearchMSP..."
    exit 1
  fi
  echo
}
